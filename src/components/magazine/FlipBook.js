...
export default class FlipBook {
  ...
  init() {
    ...
    // We bind turnPage manually so we can clean it up if needed. For now, since it binds inside its own context and won't be unmounted easily unless DOM drops, it is fine.
    pagesLeft.forEach((p) => p.addEventListener('click', () => this.turnPage('back')));
    pagesRight.forEach((p) =>
  ...
  isLastPage() {
    ...
    return this.pages.length - 1 <= activeLeft + (this.options.canClose ? 1 : 2);
  }
  ...
  isFirstPage() {
    ...
    const activeRight = activePages[this.options.canClose ? 0 : 0];
    return activeRight <= Object.keys(this.pages).findIndex(k => this.pages[k].classList.contains(this.classNames.isActive));
  }
  ...
  turnPage(direction) {
    ...
    if (activeLeft === undefined) return;
    ...
    const pagesAnimatingOut = direction === 'back' ? pagesActive.item(0) : (pagesActive.length > 1 ? pagesActive.item(1) : pagesActive.item(0));
    let pagesAnimatingIn = children.item(target);
    let pagesTarget = [pagesAnimatingIn];
    if (targetSibling !== -1 && children.item(targetSibling)) pagesTarget = [...pagesTarget, children.item(targetSibling)];
  ...
  static handleArrowKeys(options, initInterval, turnPage, Modernizr) {
    ...
    // Remove old listeners to avoid memory leaks if flipbook is re-mounted
    if (window._flipBookKeyHandler) {
        document.removeEventListener('keydown', window._flipBookKeyHandler);
    }
      
    window._flipBookKeyHandler = ({ keyCode }) => {
    ...
    document.addEventListener('keydown', window._flipBookKeyHandler);
  }
  ...
  static handleAnimationEnd(pagesAnimating, pagesActive, context) {
    pagesAnimating.forEach((page) => {
      ...
      endEvents.forEach((trans) => {
        page.addEventListener(trans, cb, {once:true});
      });
    });
  }
